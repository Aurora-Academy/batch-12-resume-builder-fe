...
export function ProjectsForm() {
  ...
  const handleGenerateDescription = async (index: number) => {
    try {
      ...
      if (!project.title || !project.technologies || project.technologies.length === 0) {
        toast({
          title: "Missing Information",
          description: "Please fill in project title and add at least one technology before generating description.",
          variant: "destructive",
        })
        return
      }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Showcase your personal projects, open source contributions, or significant work projects. This section is
          optional.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {fields.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">You can skip this section or add your projects below.</p>
          </div>
        ) : (
...
function ProjectItem({
  ...
  const removeTechnology = (techIndex: number) => {
    const newTechnologies = technologies.filter((_: string, i: number) => i !== techIndex)
    setValue(`projects.${index}.technologies`, newTechnologies)
  }
    ...
    <div className="space-y-4 p-4 border rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input {...register(`projects.${index}.title`)} placeholder="My Awesome Project" />
          {errors.projects?.[index]?.title && (
            <p className="text-sm text-destructive">{errors.projects[index]?.title?.message as string}</p>
          )}
        </div>
        <div className="space-y-2">
          <Label htmlFor={`projects.${index}.link`}>Project Link</Label>
          <Input {...register(`projects.${index}.link`)} placeholder="https://github.com/username/project" />
          {errors.projects?.[index]?.link && (
            <p className="text-sm text-destructive">{errors.projects[index]?.link?.message as string}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <Sparkle onClick={() => onGenerateDescription(index)} tooltip="Generate project description with AI" />
        </div>
        <Textarea
        ...
        {errors.projects?.[index]?.description && (
          <p className="text-sm text-destructive">{errors.projects[index]?.description?.message as string}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        {errors.projects?.[index]?.technologies && (
          <p className="text-sm text-destructive">{errors.projects[index]?.technologies?.message as string}</p>
        )}
      </div>
    </div>
  )
